...
参考 ADR: 0019 (面板设计), 0017 (编辑器架构), 0007 (同步桥接), 0015 (单线程 + 后台 reader)

> **ADR 0014 不存在**（编号跳过）。AI 原生原语 (`lint`, `dry_run`, `explain`, `diff`) 实现在 `craft-kernel` 中，无独立 ADR。`explain_node` 作为新增原语在本 milestone 引入。
...
│   ├── client.rs           # AgentClient: reqwest HTTP + SSE 解析 + AgentConfig
│   ├── config.rs           # AgentConfig: craft.toml [agent] 解析
...
impl AgentClient {
    ...
    /// 发送对话到 LLM，在后台线程执行。通过 channel 返回流事件。
    /// 若 `request_in_flight` 为 true 返回 `None`（调用方应忽略此次请求）。
    pub fn chat(
        &self,
    ...
    /// 设置 `request_in_flight` flag 并 join 活跃线程（超时 2 秒后 detach）
...
SSE 传输格式（OpenAI 兼容）：后台线程从响应体逐行读取（`BufRead::read_line`），解析 `data: ` 前缀行，空行表示事件结束，`data: [DONE]` 终止流。
...
- 同一时间最多一个活跃请求（`Arc<AtomicBool>` 保护），`chat()` 内 `compare_exchange(false, true)` 抢锁，失败返回 `None`；线程退出前重置为 `false`
- AgentPanel `show()` 调用 `chat()` 前检查返回值 — 若为 `None`（已有请求在进行），跳过本次发送，继续 drain 当前请求的 channel
...
pub struct ChangeRecord {
    pub timestamp: u64,  // 自面板打开以来的毫秒数（避免 Instant 不可序列化的问题）
    pub description: String,
}
```
...
| `read_scene` | 当前场景摘要 | `{}` | 返回 `SceneInfo` JSON（定义见下方） |
| `read_node` | 节点的完整 JSON | `{ node_id }` | 返回节点 `serde_json` |
| `propose_diff` | 建议修改场景 | `{ description, diff }` | 内联于响应，不走 tool 回环 |

**`SceneInfo` 定义**: `{ name: String, node_count: usize, node_types: Vec<String>, root_id: Option<String> }` — 场景的轻量摘要，用于 LLM 理解场景结构，避免传输完整 JSON。

**ADR 0019 工具范围差异**: ADR 0019 额外列出 `diff`（tick 间比较）和 `search_luarocks` 两个工具，E3 将其推迟到后续迭代（diff 在 E4 UX 打磨中更自然；search_luarocks 依赖模块索引，不适合 MVP）。ADR 0019 的 `SuggestionStatus::Modified` 变体同样推迟。
...
`show()` 接收 `&mut EditorState`，可以同时获取 `&Scene`（通过 `state.scene.def`）和 `&NodeRegistry`（通过 `state.engine.engine.node_registry_mut()` 的 reborrow）。无竞争：tool 执行在帧内同步完成。
...
## 7. System Prompt + LLM 指令

Agent 的 system prompt（硬编码在 `agent/mod.rs` 中）包括：

1. **角色**: "You are Craft's AI copilot. You help users build game scenes..."
2. **可用工具**: ToolDef 列表（由 ToolRegistry 生成 JSON Schema）
3. **响应格式**: "When proposing changes, include them as a JSON object with 'reply' and 'diffs' fields..."
4. **SceneDiff Schema**: 完整 `SceneDiff` 结构说明（`added_nodes`、`component_changes` 的 `ComponentChange` 变体等），让 LLM 能生成合法 diff
5. **约束**: 不读取项目目录外的文件，不直接修改磁盘，所有修改需人类审批

> **推迟**: system prompt 的外部覆盖（`craft.toml` `[agent]` 的 `system_prompt_override` 或 `system_prompt_file`）推迟到 E4。E3 仅支持硬编码 prompt，后续迭代可添加覆盖机制以加速 prompt 迭代。

---

## 8. 状态 + AgentPanelState
...
mock LLM: 测试中不使用真实 HTTP 调用。策略：

- **单元测试**（client/context/tools）：通过 trait 抽象 LLM 后端。生产代码使用 `LiveLlmBackend`（reqwest HTTP），测试使用 `MockLlmBackend`（从内联字符串或 `tests/fixtures/` JSON 文件读取响应）。
- **集成测试**（panels/agent_flow）：在 `EditorState` 中注入 pre-built `AgentPanelState`（含预设消息和建议），跳过 LLM 调用直接测试 UI 交互和 diff 应用流程。
- Mock 响应格式：每个 fixture 文件包含 `{ "response": ChatMessage, "stream_tokens": ["..."] }`，支持模拟 tool call 和非 tool call 两种模式。
...
### 线程清理
AgentClient 存储后台线程的 `JoinHandle`。编辑器关闭时（`EditorApp::on_exit`）调用 `client.shutdown()` 设置 `AtomicBool` 标志并 join 线程。超时 2 秒后 detach。
...
### apply_hot_reload 验证
`engine.apply_hot_reload()` 接收完整 `&Scene`（非 `&SceneDiff`）。验证 diff 时：(1) 克隆当前场景，(2) `apply_scene_diff` 到克隆上生成提议场景，(3) 对提议场景调用 `apply_hot_reload`。成功后再写文件和更新实际场景。
